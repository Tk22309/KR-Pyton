...
valuta_data['Рік'] = valuta_data['Дата'].apply(lambda x: int(x.split('-')[1]))  # Тепер витягуємо місяць
...
sns.lineplot(data=valuta_data, x='Рік', y='Покупка', color='blue', label='Середня покупка')
plt.xlabel('Період')
plt.ylabel(f'Ціна {valuta} в UAN')
plt.title(f'Середня річна ціна покупки {valuta} у проміжку з 2015 по 2023 рік')
plt.legend()
